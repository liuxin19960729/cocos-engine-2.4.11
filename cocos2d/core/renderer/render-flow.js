...
_proto._localTransform = function (node) {
    ...
    node._renderFlag &= ~LOCAL_TRANSFORM;// LOCAL_TRANSFORM 状态置为0
...
_proto._render = function (node) {
    let comp = node._renderComponent;// 没给节点对应的渲染组件
...
_proto._children = function (node) {
    ...
    // 父节点opacity 变化时，需要重新计算所有子节点的opacity
...
RenderFlow.visitRootNode = function (rootNode) {
    ...
    if (rootNode._renderFlag & WORLD_TRANSFORM) {// 世界位置是否改变
        _batcher.worldMatDirty ++;
